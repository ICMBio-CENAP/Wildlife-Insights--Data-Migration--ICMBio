...
dir_path <- paste(here("Datasets", "silvania"), "/", sep="")
...
projects_1 <- read_excel(paste(dir_path,"Wild_ID_FNS.xlsx",sep=""),sheet="Project")
  ...
  # in silvania there is no export 2 so:
  images <- images_1
...
prj_bu$project_name <- "Silvania"
prj_bu$project_objectives <- "Estimate Sus scrofa population density in Silvania National Forest"
...
prj_bu$project_admin <- "Mariella Butti" #projects$`Principal Investigator`
prj_bu$project_admin_email <- "mariella.butti@icmbio.gov.br" #projects$`Principal Investigator Email`
prj_bu$project_admin_organization <- "ICMBio" #projects$`Project Owner (Organization or Individual)`
...
# Load in your clean taxonomy. Clean taxononmy is created using the WI_Taxonomy.R file.
  ...
  source(here("WI_Global_Taxonomy", "elildo_create_taxonomic_mapping_silvania.R"))
...
images_taxa$new_location <- gsub("\\\\","/",images_taxa$Location)
  ...
  images_taxa$wi_path <- paste("gs://icmbio/Wild_ID_FNS","/",images_taxa$new_location,sep="")
...
dir.create(path = paste(here("batch_upload"), site_name_clean, sep="/"))
