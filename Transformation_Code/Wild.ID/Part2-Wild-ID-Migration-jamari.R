...
dir_path <- paste(here("Datasets", "jamari"), "/", sep="")

# Load Wild.ID export
images <- read.csv(here("Datasets", "jamari", "Image.csv"))
deployments <- read.csv(here("Datasets", "jamari", "Deployment.csv"))
cameras <- read.csv(here("Datasets", "jamari", "Cameras.csv"))
projects <- read.csv(here("Datasets", "jamari", "Project.csv"))
...
prj_bu$project_id <- "FNJ" # projects$`Project ID`
#prj_bu$project_name <- projects$`Project Name`
prj_bu$project_name <- "Jamari"
...
# Load in your clean taxonomy. Clean taxononmy is created using the WI_Taxonomy.R file.
  ...
  source(here("WI_Global_Taxonomy", "create-taxonomic-mapping-FNJ.R"))
...
images_taxa$new_location <- gsub("\\\\","/",images_taxa$Location)
  ...
  images_taxa$wi_path <- paste("gs://icmbio/jamari/2016","/",images_taxa$new_location,sep="")

# If all images were identified by one person, set this here. Otherwise comment this out.
#image_identified_by <- "Someones name"
