...
dir_path <- paste(here("Datasets", "Sao-Benedito-River"), "/", sep="")

# Load Wild.ID export
images <- read.csv(here("Datasets", "Sao-Benedito-River", "Image.csv"))
deployments <- read.csv(here("Datasets", "Sao-Benedito-River", "Deployment.csv"))
cameras <- read.csv(here("Datasets", "Sao-Benedito-River", "Cameras.csv"))
projects <- read.csv(here("Datasets", "Sao-Benedito-River", "Project.csv"))
...
prj_bu$project_id <- "SBR" # projects$`Project ID`
#prj_bu$project_name <- projects$`Project Name`
prj_bu$project_name <- "Sao Benedito River"
prj_bu$project_objectives <- "Wildlife survey"
...
# Load in your clean taxonomy. Clean taxononmy is created using the WI_Taxonomy.R file.
  ...
  source(here("WI_Global_Taxonomy", "create-taxonomic-mapping-SBR.R"))
...
images_taxa$new_location <- gsub("\\\\","/",images_taxa$Location)
  ...
  images_taxa$wi_path <- paste("gs://icmbio/Sao-Benedito-River","/",images_taxa$new_location,sep="")
